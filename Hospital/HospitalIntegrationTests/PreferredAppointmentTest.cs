using HospitalAPI.DTO;
using HospitalLibrary.GraphicalEditor.Model;
using HospitalLibrary.MedicalRecords.Model;
using HospitalLibrary.MedicalRecords.Model.Enums;
using Newtonsoft.Json;
using System;
using System.Collections.Generic;
using System.Net.Http;
using System.Text;
using Xunit;

namespace HospitalIntegrationTests
{
    public class PreferredAppointmentTest : IClassFixture<InjectionFixture>
    {
        private readonly InjectionFixture injection;
        public PreferredAppointmentTest(InjectionFixture injection)
        {
            this.injection = injection;
        }

        [Theory]
        [MemberData(nameof(DataSuccessfully))]
        public async System.Threading.Tasks.Task Get_Doctors_AppointmentsAsync(PreferredAppointmentRequestDTO appointmentRequestDTO)
        {
            var json = JsonConvert.SerializeObject(appointmentRequestDTO);
            var data = new StringContent(json, Encoding.UTF8, "application/json");
            var url = "api/appointment";

            var response = await injection.Client.PostAsync(url, data);

            response.EnsureSuccessStatusCode();

            Assert.NotNull(response);

        }


        public static IEnumerable<object[]> DataSuccessfully()
        {
            var retVal = new List<object[]>();

            Room room = new Room();
            room.id = 1;

            Doctor doctor = new Doctor();
            doctor.Id = "1";

            Patient patient = new Patient();
            patient.Id = "1";

            var dateString1 = "1/12/2021 8:30:00 AM";
            DateTime date1 = DateTime.Parse(dateString1,
                                      System.Globalization.CultureInfo.InvariantCulture);
            Appointment appointment1 = new Appointment(date1, 30.0, date1, -2, room, patient.Id, patient, doctor.Id, doctor);

            var dateString2 = "1/12/2021 9:00:00 AM";
            DateTime date2 = DateTime.Parse(dateString1,
                                      System.Globalization.CultureInfo.InvariantCulture);
            Appointment appointment2 = new Appointment(date2, 30.0, date2, -2, room, patient.Id, patient, doctor.Id, doctor);

            var dateString3 = "1/12/2021 9:30:00 AM";
            DateTime date3 = DateTime.Parse(dateString1,
                                      System.Globalization.CultureInfo.InvariantCulture);
            Appointment appointment3 = new Appointment(date3, 30.0, date3, -2, room, patient.Id, patient, doctor.Id, doctor);

            List<Appointment> doctorsAppointments = new List<Appointment>();
            doctorsAppointments.Add(appointment1);
            doctorsAppointments.Add(appointment2);
            doctorsAppointments.Add(appointment3);



            PreferredAppointmentRequestDTO appointmentRequestDTO = new PreferredAppointmentRequestDTO(
                    "1/12/2021", doctor.Id, DoctorType.surgeon, "doctor"
                );

            retVal.Add(new object[] { doctorsAppointments, appointmentRequestDTO });

            return retVal;
        }
    }
}
